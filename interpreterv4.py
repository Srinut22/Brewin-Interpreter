...
class Interpreter(InterpreterBase):
    ...
    def __set_up_function_table(self, ast):
        ...
        for func_def in ast.get("functions"):
            ...
            self.func_name_to_ast[func_name][num_params] = Closure(
                func_def, empty_env)
    ...
    def __run_statements(self, statements):
        ...
        for statement in statements:
            ...
            if statement.elem_type == Interpreter.FCALL_DEF:
            ...
            if statement.elem_type == Interpreter.MCALL_DEF:
    ...
    def __call_method(self, call_ast):
        ...
        obj_value = self.env.get(obj_name)
        if obj_value is None:
            super().error(ErrorType.NAME_ERROR,
                          f"Variable {obj_name} not found")
        elif obj_value.type() != Type.OBJECT:
            super().error(ErrorType.TYPE_ERROR,
                          "Trying to call method on non-object")

        self.env.push()
        self.env.create(Interpreter.THIS_DEF, obj_value)

        method_closure = obj_value.value().get(method_name)
        if method_closure is None:
            super().error(ErrorType.NAME_ERROR,
                          f"Method {obj_name}.{method_name} not found")
        elif method_closure.type() != Type.CLOSURE:
            super().error(ErrorType.TYPE_ERROR,
                          "Trying to call non-function/closure")
        return_value = self.__call_func(call_ast, method_closure.value())
        if Interpreter.THIS_DEF in self.env.environment[-1].keys():
            del self.env.environment[-1][Interpreter.THIS_DEF]

        self.env.pop()
        return return_value

    def __call_func(self, call_ast, method_closure=None):
        ...
        if call_ast.elem_type == Interpreter.MCALL_DEF:
            target_closure = method_closure
        else:
            target_closure = self.__get_func_by_name(
                func_name, len(actual_args))
    ...
    def __assign_object(self, assign_ast):
        obj_name, var_name = assign_ast.get("name").split('.')
        obj_value = self.env.get(obj_name)
        if obj_value is None:
            super().error(ErrorType.NAME_ERROR,
                          f"Variable {obj_name} not found")
            ...
            super().error(ErrorType.TYPE_ERROR,
                          f" Dot operator used on non-object {obj_name}")

        var_value = self.__eval_expr(assign_ast.get("expression"))
        if var_name == Interpreter.PROTO_DEF:
            if var_value.type() != Type.OBJECT and var_value.type() != Type.NIL:
                super().error(ErrorType.TYPE_ERROR, "Can't set proto to non-object type")
            obj_value.value().set_prototype(var_value)
        else:
            if var_value.type() == Type.CLOSURE or var_value.type() == Type.OBJECT:
                obj_value.value().set(var_name, var_value, isCopied=False)
            else:
                obj_value.value().set(var_name, var_value, isCopied=True)
        self.env.set(obj_name, obj_value)

    def get_new_object(self, target_value_obj):
        temp = Object()
        for key in target_value_obj.value().environment.keys():
            temp.set(key, target_value_obj.value().environment[key])

        object_value = Value(Type.OBJECT, temp)
        return object_value

    def __assign(self, assign_ast):
        var_name = assign_ast.get("name")
        if '.' in var_name:
            self.__assign_object(assign_ast)
        else:
            src_value_obj = (
                self.__eval_expr(assign_ast.get("expression")))
            target_value_obj = self.env.get(var_name)
            if target_value_obj is None:
                self.env.set(var_name, src_value_obj)
            elif target_value_obj.type() == Type.OBJECT:
                new_value_obj = self.get_new_object(target_value_obj)
                self.env.modify_objects(
                    var_name, target_value_obj, new_value_obj)
                self.env.set(var_name, src_value_obj)
            else:
    ...
    def __eval_member_variable_name(self, name_ast):
        obj_name, var_name = name_ast.get("name").split('.')
        obj_value = self.env.get(obj_name)
        if obj_value is None:
            super().error(ErrorType.NAME_ERROR,
                          f"Variable/function {obj_name} not found")
        elif obj_value.type() != Type.OBJECT:
            super().error(ErrorType.TYPE_ERROR,
                          f"Dot operator used on non-object {obj_name}")
        var_value = obj_value.value().get(var_name)
        if var_value is None:
            super().error(ErrorType.NAME_ERROR,
                          f"Variable/function {var_name} not found")
        return var_value

    def __eval_name(self, name_ast):
        var_name = name_ast.get("name")
        if '.' in var_name:
            return self.__eval_member_variable_name(name_ast)
